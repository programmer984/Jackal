...
public class ClientInfo {
    ...
    static EncryptionTool asymmetric;
    ...
    public ClientInfo(String clientName, Iterable<InetAddress> localAddresses, int localPort,
                      InetSocketAddress internetAddress, DatagramSocket socket) {
        if (clientName == null || localAddresses == null || internetAddress == null
                || localPort <= 0) {
            throw new IllegalArgumentException();
        }
        this.creationTimestamp = calculateTimeStamp();
    ...
    @JsonIgnore
    public PublicKey getKeyForDecrypt() throws Exception {
        return asymmetric.getPublicKey(publicKey);
    }
    ...
    public static long calculateTimeStamp() {
        return org.joda.time.Instant.now().getMillis() / 1000;
    }
