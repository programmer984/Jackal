...
public class VideoFramePacket extends AbstractPacket {
    ...
    public static final int MTU = ((1024 - HEADER_LENGTH - TLC_LENGTH) * 8) - Configuration.AES_SIZE;
    ...
    public static int getPacketId(byte[] packets, int packetOffset) {
        return Utils.bufToI32(packets, packetOffset + ID_OFFSET);
    }
    ...
    @Override
    public byte[] toArray() {
        int originalSize = TLC_LENGTH + HEADER_LENGTH + partSize;
        int packetSize = semicolon(originalSize);
        byte[] buf = new byte[packetSize];
        Utils.u8ToBuf(totalApproximateSize, buf, APPROX_SIZE_OFFSET);
        Utils.i32ToBuf(id, buf, ID_OFFSET);
        Utils.u8ToBuf(partsCount, buf, PARTS_COUNT);
        Utils.u8ToBuf(partIndex, buf, PART_INDEX);
        Utils.u8ToBuf(frameConfig, buf, FRAME_CONFIG);
        Utils.bufToBuf(data, partOffset, partSize, buf, VIDEO_DATA_OFFSET);
    ...
    public String getDescription() {
        return String.format("version {} part {}/{}", id, partIndex + 1, partsCount);
    }
