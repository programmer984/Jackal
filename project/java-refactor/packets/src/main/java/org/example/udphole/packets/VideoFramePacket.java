...
public class VideoFramePacket extends AbstractPacket {
    ...
    public static final int MTU = ((1024 - HEADER_LENGTH - TLC_LENGTH) * 8) - CryptoUtils.getAesSize();
    ...
    public byte[] toArray() {
        ...
        ByteUtils.u8ToBuf(totalApproximateSize, buf, APPROX_SIZE_OFFSET);
        ByteUtils.i32ToBuf(id, buf, ID_OFFSET);
        ByteUtils.u8ToBuf(partsCount, buf, PARTS_COUNT);
        ByteUtils.u8ToBuf(partIndex, buf, PART_INDEX);
        ByteUtils.u8ToBuf(frameConfig, buf, FRAME_CONFIG);
        ByteUtils.bufToBuf(data, partOffset, partSize, buf, VIDEO_DATA_OFFSET);
