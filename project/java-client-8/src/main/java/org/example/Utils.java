...
public class Utils {
    ...
    public static byte[] makeAesPadding(byte[] notEncrypted) {
        int remainder = notEncrypted.length % Configuration.AES_SIZE;
        if (remainder != 0) {
            int padding = Configuration.AES_SIZE - remainder;
            return Arrays.copyOf(notEncrypted, notEncrypted.length + padding);
        }
        return notEncrypted;
    }

    public static int getPaddedSize(int size) {
        int remainder = size % Configuration.AES_SIZE;
        if (remainder != 0) {
            int padding = Configuration.AES_SIZE - remainder;
            return size + padding;
        }
    ...
    public static Integer searchSequense(byte[] src, int startOffset, byte[] target) {
        for (int i = startOffset; i < src.length; i++) {
            if (src[i] == target[0] && i + target.length < src.length) {
                boolean found = true;
