...
JNIEXPORT void JNICALL Java_com_example_dataprovider_service_Codec_init(JNIEnv *env, jobject thiz,
...
Java_com_example_dataprovider_service_Codec_disposeLowLevel(JNIEnv *env, jobject thiz) {
    shouldWork = 0;
...
void writeResultVideoFrame(byte *header, byte *data, int dataSize) {
    ...
    write(pipeOut, header, HeaderSize);

    memcpy(&frameSize, (byte *) &dataSize, 4);
    write(pipeOut, &frameSize, 4);
